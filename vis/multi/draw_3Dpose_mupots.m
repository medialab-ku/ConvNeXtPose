function draw_3Dpose_mupots()
    ...
    root_path = '/media/sonnguyen/DATA/Work/Data/3DMPPE_POSENET_RELEASE/data/MuPoTS/data/MultiPersonTestSet/';
    save_path = './vis/';
    num_joint =  17;
    prev_folder_id = -1;
    pred_3d_kpt_pivot = -1;
    ...
    while ischar(img_name)
        ...
        if folder_id ~= prev_folder_id
            pred_3d_kpt_pivot = pred_3d_kpt;
            prev_folder_id = folder_id;
        end
        img = draw_2Dskeleton(img_path,pred_2d_kpt,num_joint,skeleton,colorList_joint,colorList_skeleton);
        %img = imread(img_path);
        f = draw_3Dskeleton(img, pred_3d_kpt,num_joint,skeleton,colorList_joint,colorList_skeleton, pred_3d_kpt_pivot);
